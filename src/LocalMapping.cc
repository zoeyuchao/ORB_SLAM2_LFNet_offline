...
void LocalMapping::CreateNewMapPoints()
{
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            const float &sigmaSquare1 = mpCurrentKeyFrame->mvLevelSigma2[kp1.octave];// zoe 20181021 取1
            ...
            if(!bStereo1)
            {
                ...
                if((errX1*errX1+errY1*errY1)>5.991*sigmaSquare1)// zoe 超参数不知道怎么来的
                    continue;
            }
            else
            {
            ...
            const float sigmaSquare2 = pKF2->mvLevelSigma2[kp2.octave];// zoe 要大改
