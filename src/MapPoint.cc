...
//zoe 20181017
void MapPoint::ComputeDistinctiveDescriptorsLFNet()
{
    // Retrieve all observed descriptors
    ...
    for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
    {
        ...
        if(!pKF->isBad())
        {
            //cout << "id = "<< mit->second<<endl;
            vDescriptors.push_back(pKF->mvDspts[mit->second]);
        }
    }
    if(vDescriptors.empty())
        return;
    ...
    for(size_t i=0;i<N;i++)
    {
        ...
        for(size_t j=i+1;j<N;j++)
        {
            float distij = ORBmatcher::DescriptorDistanceLFNet_float(vDescriptors[i],vDescriptors[j]);
            Distances[i][j]=distij;
            Distances[j][i]=distij;
        }
    }
    ...
    // Take the descriptor with least median distance to the rest
    float FLOAT_MAX=5.0;//zoe 这里还有一个阈值,注意!
    float BestMedian = FLOAT_MAX;//zoe
    int BestIdx = 0;
    for(size_t i=0;i<N;i++)
    {
...
std::vector<float> MapPoint::GetDescriptorLFNet()
{
    unique_lock<mutex> lock(mMutexFeatures);
